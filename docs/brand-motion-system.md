...
The system pairs circular fields with horizontal rails. Circles open space; rails build, cover, and clear it. Reveals share an 18-frame entrance and 12-frame exit at 30 fps, with proportional timing on short clips. Covering wipes use staggered coverage and a fully opaque center interval; the fine pixel transition reveals and fades a stationary grid. Loops use a periodic frame phase.
