...
export function ColorBar({overlay, frame, fps}: {overlay: Overlay; frame: number; fps: number}) {
  ...
  return <div data-color-bar={overlay.kind} style={{position: 'absolute', isolation: 'isolate', left: 0, right: 0, height, top: position === 'top' ? 0 : position === 'center' ? (720 - height) / 2 : undefined, bottom: position === 'bottom' ? 0 : undefined, overflow: 'hidden', opacity: overlay.opacity ?? 1}}>
