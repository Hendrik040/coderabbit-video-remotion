...
export function NameIntroControls({asset, onChange}: {asset: Overlay; onChange: (patch: Partial<Overlay>) => void}) {
  const [target, setTarget] = useState<'name' | 'details' | 'text'>('name');
  const labels = {name: 'Name fill', details: 'Details fill', text: 'Text'};
  const label = labels[target];
  const textColor = asset.textColor ?? '#121014';
  const colors = {name: asset.accent, details: asset.detailBackground ?? '#25E2A8', text: textColor};
  const value = colors[target];
  const changeColor = (color: string) => onChange(target === 'name' ? {accent: color} : target === 'details' ? {detailBackground: color} : {textColor: color});
  ...
  return <>
    ...
    <div className="field-label"><span>Colors</span><div className="placement-picker name-color-targets" role="group" aria-label="Intro color to edit">{(['name', 'details', 'text'] as const).map(color => <button key={color} aria-label={labels[color]} aria-pressed={target === color} onClick={() => setTarget(color)}><span className="palette-current-chip" style={{backgroundColor: colors[color]}} aria-hidden="true"/>{color}</button>)}</div></div>
