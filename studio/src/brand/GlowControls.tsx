...
function NumberControl({label, name, value, min, max, step = 1, unit = '%', onChange}: ControlProps) {
...
function SliderControl({label, name, value, min, max, step = 1, unit = '%', onChange}: ControlProps) {
  const id = useId();
...
export function GlowControls({asset, onChange}: {asset: Overlay; onChange: (patch: Partial<Overlay>) => void}) {
  ...
  return <div className="glow-controls">
    <NumberControl label="Cycle length" name="Glow cycle length" value={asset.loopDuration ?? 16} min={4} max={32} step={0.5} unit="s" onChange={loopDuration => onChange({loopDuration})}/>
    <details className="glow-control-section" open><summary><span>Lighting</span><ChevronDown size={13}/></summary>
    ...
    <SliderControl label="Backdrop opacity" name="Backdrop opacity" value={(asset.opacity ?? 1) * 100} min={10} max={100} onChange={opacity => onChange({opacity: opacity / 100})}/>
