...
export function AssetControls({asset, onChange}: {asset: Overlay; onChange: (patch: Partial<Overlay>) => void}) {
  ...
  return <>
    ...
    {template && !isColorBar(asset.kind) && <div className="field-label"><span>Colorway</span><div className="placement-picker" role="group" aria-label="Asset colorway">{(['dark', 'light'] as const).map(colorway => <button key={colorway} aria-pressed={(asset.colorway ?? 'dark') === colorway} onClick={() => onChange({colorway})}>{colorway}</button>)}</div></div>}
    {(isTransition(asset.kind) || asset.kind === 'signal-loop') && <div className="field-label"><span>Direction</span><div className="placement-picker" role="group" aria-label="Asset direction">{(['left', 'right'] as const).map(direction => <button key={direction} aria-pressed={(asset.direction ?? 'right') === direction} onClick={() => onChange({direction})}>{direction}</button>)}</div></div>}
    {isColorBar(asset.kind) ? <ColorBarControls asset={asset} onChange={onChange}/> : <PalettePicker value={asset.accent} onChange={accent => onChange({accent})}/>}
  </>;
}
