...
const kindNames: Record<OverlayKind, string> = {...Object.fromEntries(broadcastKinds.map(k => [k, broadcastTemplates[k].name])) as Record<OverlayKind, string>,hero: 'Change Stack glow', terminal: 'Terminal', agentflow: 'Agent workflow', code: 'Code panel', diagram: 'API flow', callout: 'Callout'};
const kindIcons = {hero: LoopIcon, ident: Film, presenter: MessageSquare, headline: Code2, triage: Layers3, stack: GitBranch, ticker: MoveHorizontal, bug: ShieldCheck,terminal: Terminal, agentflow: Workflow, code: Code2, diagram: GitBranch, callout: MessageSquare};
const colors = [{value: '#FF570A', label: 'CodeRabbit orange'}, {value: '#25E2A8', label: 'Mint'}, {value: '#687FF5', label: 'Cobalt'}, {value: '#EEEDEF', label: 'Mauve white'}];
...
export function App() {
  ...
  const [libraryGroup, setLibraryGroup] = useState<AssetType>(() => assetType(project.overlays[0]?.kind ?? 'ident'));
  ...
  return <div className="app-shell">
    ...
    <div className="workspace">
      <aside className="library-panel">
        <div className="panel-heading"><span>The broadcast kit</span><Layers3 size={16}/></div>
        ...
        {libraryTab === 'components' ? <>
          <p className="panel-intro">One station. A consistent voice.<br/>Select a template. Make it yours.</p><div className="kit-switch" role="group" aria-label="Asset type"><button aria-pressed={libraryGroup === 'linear'} onClick={() => setLibraryGroup('linear')}>Linear <span>{assetCollections.linear.length}</span></button><button aria-pressed={libraryGroup === 'looping'} onClick={() => setLibraryGroup('looping')}>Looping <span>{assetCollections.looping.length}</span></button></div>
          <div className="component-library">{assetCollections[libraryGroup].map(kind => {const Icon = kindIcons[kind]; return <button className={`component-card ${selected?.kind === kind ? 'is-selected' : ''}`} key={kind} aria-label={`Select ${kindNames[kind]} component`} onClick={() => {const existing = project.overlays.find(o => o.kind === kind); existing ? selectOverlay(existing) : addOverlay(kind);}}>
            <Miniature kind={kind}/><div className="component-caption"><Icon size={15}/><span>{kindNames[kind]}</span><span className="component-language">{assetType(kind) === 'looping' ? 'LOOP' : isBroadcast(kind) ? broadcastTemplates[kind].code : (kind === 'diagram' || kind === 'agentflow') ? 'FLOW' : 'TEXT'}</span></div>
          </button>;})}</div>
      ...
      <main className="editor-main">
        <div className="editor-topbar"><div className="editor-title"><span className="section-eyebrow">PREVIEW</span><span>{onlyLooping ? 'Background loop.' : 'The Review Desk.'}</span></div><div className="stage-actions"><select aria-label="Load studio preset" value="" onChange={e => {if (e.target.value) resetSample(e.target.value as "broadcast" | "walkthrough" | "technical" | "loop");}}><option value="">Load a preset</option><option value="broadcast">Broadcast rundown</option><option value="loop">Change Stack glow loop</option><option value="walkthrough">Developer walkthrough</option><option value="technical">Code & API explainer</option></select><button className={`tracking-toggle ${project.showTracking ? 'is-on' : ''}`} aria-pressed={project.showTracking} onClick={() => commit(p => ({...p, showTracking: !p.showTracking}))}><ScanLine size={15}/><span>Tracking</span><i/></button></div></div>
        <div className="preview-space">
          <div className="preview-meta"><span><i/>{onlyLooping ? 'LOOPING ASSET / SEAMLESS' : project.broadcast && project.sampleMode ? 'BROADCAST / SAMPLE RUNDOWN' : project.sampleMode ? 'SAMPLE / SIMULATED MOTION' : project.mediaName}</span><span>1280 × 720 <span className="meta-separator">/</span> 30 FPS</span></div>
      ...
      <aside className="inspector-panel">
        ...
        {selected ? <>
          <div className="selected-component"><div className={`selected-icon icon-${selected.kind}`}>{React.createElement(kindIcons[selected.kind], {size: 21})}</div><div><strong>{kindNames[selected.kind]}</strong><span>{isLooping ? 'LP-01 / Looping background' : template ? `${template.code} / Linear asset` : 'Linear asset'}</span></div><button className="icon-button" aria-label={selected.enabled ? 'Hide selected overlay' : 'Show selected overlay'} onClick={() => updateOverlay({enabled: !selected.enabled})}>{selected.enabled ? <Eye size={16}/> : <EyeOff size={16}/>}</button></div>
          <section className="inspector-section"><h2><Hand size={14}/> {isLooping ? 'Loop timing' : template ? 'Playout timing' : 'Gesture binding'}</h2>{isLooping ? <p className="binding-explanation">Repeats seamlessly for the layer’s full duration. No entrance or exit fade.</p> : template ? <p className="binding-explanation">18 frames in. Readable hold. 12 frames out. Short clips compress the motion automatically.</p> : <><label className="field-label">Behavior<select aria-label="Overlay behavior" value={selected.binding} onChange={e => updateOverlay({binding: e.target.value as Binding})}><option value="cue">Reveal on cue</option><option value="progress">Hand position → progress</option><option value="follow">Follow hand</option></select></label>
            ...
            {!isLooping && <label className="field-label">Set start from a cue<select aria-label="Bind overlay to gesture cue" value="" onChange={e => {const cue = project.cues.find(c => c.id === e.target.value); if (cue) {const start = clamp(cue.time, 0, project.duration - 0.1); updateOverlay({start, duration: Math.min(selected.duration, project.duration - start)}); seek(start + 0.5);}}}><option value="">Choose a detected cue</option>{project.cues.map(cue => <option key={cue.id} value={cue.id}>{gestureLabel[cue.gesture]} · {cue.time.toFixed(1)}s</option>)}</select></label>}
