...
export const projectSchema = z.object({
  ...
  overlays: z.array(z.object({id: z.string().max(100), kind: z.enum(['code', 'diagram', 'callout', 'terminal', 'agentflow', 'presenter', 'headline', 'triage', 'stack', 'ticker', 'bug', 'ident', 'hero', 'name-intro', 'logo-reveal', 'circle-wipe', 'stack-wipe', 'color-bar-wipe', 'pixel-glow-wipe', 'type-reveal', 'brand-signoff', 'signal-loop', 'color-bar-reveal', 'color-bar-loop', 'color-bar-transition']), enabled: z.boolean(), company: z.string().max(72).optional(), detailBackground: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(), textColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(), pixelSize: wipePixelSize.optional(), scatter: z.number().finite().min(0).max(1).optional(), lighting: lightingSchema.optional(), vignette: vignetteSchema.optional(), barHeight: z.number().finite().min(4).max(144).optional(), barPosition: z.enum(['top', 'center', 'bottom']).optional(), barColors: z.array(z.string().regex(/^#[0-9a-fA-F]{6}$/)).length(10).optional(), colorway: z.enum(['dark', 'light']).optional(), direction: z.enum(['left', 'right']).optional(), kicker: z.string().max(40).optional(), loopDuration: z.number().finite().min(4).max(32).optional(), intensity: z.number().finite().min(0.25).max(1.5).optional(), opacity: z.number().finite().min(0.1).max(1).optional(), title: z.string().max(150), body: z.string().max(3000), start: z.number().min(0).max(MAX_VIDEO_SECONDS), duration: z.number().min(0.1).max(MAX_VIDEO_SECONDS), binding: z.enum(['cue', 'progress', 'follow']), placement: z.enum(['left', 'right', 'center']), accent: z.string().regex(/^#[0-9a-fA-F]{6}$/), scale: z.number().min(0.5).max(1.3)})).max(12),
}).superRefine((p, ctx) => {
